...
const pages = {
  terms: {
    ...
    body: [
      ...
      ["Conta e segurança", "O usuário é responsável por manter credenciais seguras, autorizar apenas operadores legítimos e comunicar qualquer suspeita de acesso indevido. Credenciais de API devem permanecer exclusivamente em ambiente server-side."],
      ["Operações", "Pagamentos, saldos, reservas, liquidações, estornos e outras movimentações são registrados pelo XPayments Core. A interface PagarPIX não constitui um ledger financeiro independente."],
  ...
  privacy: {
    label: "PRIVACIDADE",
    title: "Dados tratados com propósito e controle.",
    intro: "Resumo preliminar de como os dados podem circular entre o PagarPIX e o XPayments Core.",
    body: [
      ["Dados tratados", "Podem ser tratados dados cadastrais de merchants, usuários autorizados, compradores, transações, dispositivos, logs técnicos e informações necessárias à prevenção de fraude e ao cumprimento legal."],
      ...
      ["Arquitetura", "O frontend não armazena credenciais de providers nem mantém um banco financeiro paralelo. Dados financeiros canônicos pertencem ao XPayments Core."],
  ...
  cookies: {
    ...
    body: [
      ...
      ["Controle", "O usuário poderá gerenciar cookies não essenciais através do painel de preferências quando essas tecnologias forem ativadas."]
    ]
  }
} as const;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return page ? { title: page.label, alternates: { canonical: `/legal/${slug}` }, robots: { index: false, follow: false } } : {};
}
...
export default async function LegalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!page) notFound();
  return (
    <PublicPage eyebrow={page.label} title={page.title} intro={page.intro} notice={<><strong>Validação jurídica pendente</strong><span>Este conteúdo é preliminar, não está pronto para contratação pública e deve ser validado antes do lançamento comercial.</span></>}>
